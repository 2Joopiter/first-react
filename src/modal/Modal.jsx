...
/*
  ...
  -컴포넌트 생성(Mount)
    ...
    : 실사례1- 모달창 생성시 강제로 스크롤바 제거 해야될 때
    : 실사례2- resize, scroll, mousewheel, mousemove같은 window 전역에 직접 이벤트 연결해야되는 특정 컴포넌트 전용함수를 등록할 때
    : 실사례3(중요)- data fetching같이 반복 호출할 필요가 없는 무거운 연산을 처리할 때
  ...
  -컴포넌트 소멸(Unmount)
    ...
    : 실사례1- 모달창 제거시 다시 스크롤바 활성화 시킬 때
    : 실사례2-window 전역에 연결되어있는 특정 컴포넌트 함수를 제거할 때
    : 
*/
